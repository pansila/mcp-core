    ...
    fn parse_sse_message(event: &str) -> Option<SseEvent> {
        ...
        if !current_data.is_empty() {
            let result = match (event_type.as_ref(), Some(&current_data)) {
                (Some(endpoint), Some(url)) if endpoint == "endpoint" => {
                    Some(SseEvent::Session(url.to_string()))
                }
        ...
        while let Some(pos) = buffer.find("\n\n") {
            ...
            if let Some(sse_event) = Self::parse_sse_message(&complete_event) {
                match sse_event {
                    ...
                    SseEvent::Session(url) => {
                        debug!("Received session URL: {}", url);
                        *session_url.lock().await = Some(url);
                    }
                }
            }
        }
...
impl Transport for ClientSseTransport {
    ...
    async fn open(&self) -> Result<()> {
        ...
        let handle = tokio::spawn(async move {
            ...
            // Handle first message to get session URL
            if let Some(first_chunk) = event_stream.next().await {
                match first_chunk {
                    Ok(bytes) => Self::handle_sse_chunk(bytes, &tx, &session_url, &buffer).await?,
                    Err(e) => {
            ...
            while let Some(chunk) = event_stream.next().await {
                if let Ok(bytes) = chunk {
                    if let Err(e) = Self::handle_sse_chunk(bytes, &tx, &session_url, &buffer).await
                    {
